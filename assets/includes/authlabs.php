<?php
/* Return just the base domain name */
...
class Authoritylabspartner {
	 ...
	 */
	public function parseRanks($json_url, $auth_token, $industry, $keyword)
	{
		$arr_rankings = '';
		...
		$arr_rankings = $this->_parseit($json_data, $industry, $keyword);

		return $arr_rankings;
	}

	private function _parseit($json_data, $industry, $keyword)
	{
		...
		if (is_object($json_data)) {
			...
			foreach ($serp as $key => $val) {
				$domain = trim($val->base_domain);
				$current_page = $val->page;
				$arr_rankings[] = "$industry - $domain - $current_page - $keyword";
			}
				return $arr_rankings;
		}
		else
